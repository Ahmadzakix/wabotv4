...
const { wait, simih, getBuffer, h2k, generateMessageID, getGroupAdmins, groupLimit, memberLimit, getRandom, start, info, success, close } = require('./lib/functions')
...
async function starts() {
	...
	fs.existsSync('./HLXBOT.json') && nzwa.loadAuthInfo('./HLXBOT.json')
	nzwa.on('connecting', () => {
		start('2', 'Connecting...')
	})
	nzwa.on('open', () => {
	...
	await nzwa.connect({timeoutMs: 30*1000})
        fs.writeFileSync('./HLXBOT.json', JSON.stringify(nzwa.base64EncodedAuthInfo(), null, '\t'))
	...
	nzwa.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				only: {
					group: '[⚠️] Perintah ini hanya bisa di gunakan dalam group!',
					ownerG: '[⚠️] Perintah ini hanya bisa di gunakan oleh owner group!',
					ownerB: '[⚠️] Perintah ini hanya bisa di gunakan oleh owner bot!',
					premium: '*[⚠️]Maaf Perintah Ini Hanya Dapat Di Gunakan Oleh User Premium!!*',
					admin: '[⚠️] Perintah ini hanya bisa di gunakan oleh admin group!',
					Badmin: '[⚠️] Perintah ini hanya bisa di gunakan ketika bot menjadi admin!',
    ...
    nzwa.sendMessage(from, options, text, { quoted: mek } )
			...
			case 'quotes':
				...
				case 'addprem':
					...
					addpremium = addpremi
					reply(`𝗕𝗘𝗥𝗛𝗔𝗦𝗜𝗟 𝗠𝗘𝗡𝗔𝗠𝗕𝗔𝗛𝗞𝗔𝗡${addpremium} 𝗞𝗘 𝗨𝗦𝗘𝗥 𝗣𝗥𝗘𝗠𝗜𝗨𝗠*\n\n 𝗦𝗘𝗟𝗔𝗠𝗔𝗧 𝗠𝗘𝗡𝗚𝗚𝗨𝗡𝗔𝗞𝗔𝗡 𝗙𝗜𝗧𝗨𝗥 𝗣𝗥𝗘𝗠𝗜𝗨𝗠:)`)
					break
				case 'removeprem':
				case 'dellprem':
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'join':
					...
					let chekgrup = await nzwa.inviteInfo(linkgrup)
					if (!islink) return reply(from, 'Maaf link group-nya salah! ', id)
					if (isOwnerBot) {
					  await nzwa.joinGroupViaLink(linkgrup)
					    .then(async () => {
					      nzwa.sendMessage(from, 'Berhasil join grup via link!', text)
					    })
					} else {
					  let cgrup = await nzwa.getAllGroups()
					  if (cgrup.length > groupLimit) return nzwa.reply(from, `Sorry, the groups is not valid `, id)
					  if (cgrup.size < memberLimit) return nzwa.reply(from, `Sorry, Bot wil not join if the group members do not exceed ${memberLimit} people`, id)
					  await nzwa.joinGroupViaLink(linkgrup)
					    .then(async () => {
					...
					case 'totaluser':
        bo = args[0]
        let userdat = `---=[ Total User ${botName} ]=---\n`
        nom++
        userdat += `[${nom}] @${_registered.id.split('@')}\n`
        await reply(userdat)
        break
				case '3dtext':
	...
	case 'hidetag':
		   ...
		   await limitAdd(sender)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					setTimeout( () => {
					nzwa.sendMessage(from, caklontong, text, {quoted: mek})
					}, 0) // 1000 = 1s,
					await limitAdd(sender) 
					break 
                                case 'tiktokstalk':
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Kirim perintah ${prefix}marvellogo teks, contoh ${prefix}marvellogo HLXBOT Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'trendtwit':
				...
				case 'tulis':
					...
					nzwa.sendMessage(from, buff, image, {quoted: mek, caption: 'Nih lain kali nulis sendiri jangan males'})
					 break
	case 'kbbi':
            nzwa.updatePresence(from, Presence.composing) 
          ...
          if (!isRegister) return reply(ind.noregis())
            ...
            await nzwa.sendMessage(from, `Pong!!!!\nSpeed: ${processTime(time, moment())} _Second_`)
					break
				    case 'lirik':
				...
				nzwa.sendMessage(from, thum, image, { quoted : mek, caption: teks })
				break
			 case 'igstalk':
                   if (!isRegister) return reply(ind.noregis())
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
                     teks = body.slice(9)
                     anu = await fetchJson(`https://api.zeks.xyz/api/igstalk?username=${teks}&apikey=apivinz`, {method: 'get'})
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           case 'quran':
					...
					break
           case 'nekonime':
           data = await fetchJson('https://videfikri.com/api/anime/neko')
           if (!isRegister) return reply(mess.only.daftarB)
           if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan > 300) return nzwa.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
...
har = body.slice(12)
reply('「❗」HARTA TAHTA LAGI DI PROSES') 
...
break
          ...
          case 'gantengcek':
					...
					const gan =['10','33','20','40%','50','60','70','62','74','83','97%','100','29','94','75','82','41','39']
					...
					if (!isRegister) return reply(mess.only.daftarB)
					cantik = body.slice(1)
					const can =['10','33','20','40%','50','60','70','62','74','83','97%','100','29','94','75','82','41','39']
				...
				case 'logowolf2':
					...
					teks = `*➸ Nama bot* : HLX-BOT\n*➸ OWNER* : 𝗔𝗛𝗠𝗔𝗗𝗭𝗔𝗞𝗜\n*➸ AUTHOR* : ${ownerName}\n*➸ Nomor Bot* : @${me.jid.split('@')[0]}\n*➸ Prefix* : ${prefix}\n*➸ Total Block Contact* : ${blocked.length}\n*➸ The bot is active on* : ${kyun(uptime)}`
					buffer = await getBuffer('305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg')
					nzwa.sendMessage(from, buffer, image, {quoted: mek})
					 await limitAdd(sender)
					break
                              case 'pornhub':
		  	...
		  	if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				var porn = gh.split("&")[0];
				var hub = gh.split("&")[1];
				if (args.length < 1) return reply(`「❗」Contoh : ${prefix}pornhub hlx & Hub`)
				reply(ind.wait())
				buffer = await getBuffer(`https://api.zeks.xyz/api/phlogo?text1=${porn}&text2=${hub}&apikey=apivinz`)
				nzwa.sendMessage(from, buffer, image, {quoted: mek})
				await limitAdd(sender)
				break
                case 'truth':
            ...
            break
				        ...
				        uptime = process.uptime()
                                ...
                                case 'lionlogo':
                                        ...
                                        var teks1 = gh.split("|")[0];
                                        var teks2 = gh.split("|")[1];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}lionlogo HLX|BOT')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'jsholat':
                                        ...
                                        anu = await fetchJson(`https://tobz-api.herokuapp.com/api/jadwalshalat?q=${loc}&apikey=BotWeA`, {method: 'get'})
                                        mbteks = ` *JAM SHALAT* \n Daerah : ${loc} \n         
                     Ashar : ${anu.Ashar} \n   Dhuha : ${anu.Dhuha} \n   Dzuhur : ${anu.Dzuhur} \n   Imsyak : ${anu.Imsyak} \n Isya : ${anu.Isya} \n Maghrib : ${anu.Maghrib} \n Subuh : ${anu.Subuh}`
                                ...
                                case 'jokerlogo':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('teksnya mana um\nContoh: ${prefix}jokerlogo HLXBOT')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					break
                                case 'pubglogo':
                                        ...
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        anu = await fetchJson(`https://api.zeks.xyz/api/pubglogo?text1=${text1}&text2=${text2}&apikey=apivinz`, {method: 'get'})
                                        buffer = await getBuffer(anu.result)
